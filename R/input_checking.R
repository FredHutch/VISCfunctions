...
#' @param scalar logical value indicating if the checked value should only have a length of 1
...
#' @param allow_dup logical value indicating if allowing duplicate values
#'
#' @examples
...
.check_numeric_input = function(x, lower_bound = NULL, upper_bound = NULL,
                                scalar = FALSE, whole_num = FALSE,
                                allow_NA = FALSE, allow_dup = TRUE){
  param_name <- deparse(match.call()[[2]])
  if (length(dim(x)) > 1)
    stop('"', param_name, '" must be a vector (one-dimensional object)')
  if (length(x) == 0)
    stop('"', param_name, '" length must be > 0')
  if (scalar & length(x) != 1)
    stop('"', param_name, '" length must be 1 since expecting scalar')
  if (!allow_NA) x <- x[!is.na(x)]
  if (!allow_dup & any(duplicated(x) & !is.na(x)))
    stop('"', param_name, '" must have no duplicates')
  if (length(x) == 0) stop('"', param_name,
                           '" must have at least one non-NA value')
  if (!is.numeric(x) && !all(is.na(x)))
    stop('"', param_name, '" must be a numeric vector')
  if (!is.null(lower_bound) & any(x < lower_bound))
    stop('"', param_name, '" must be greater than or equal to ', lower_bound)
  if (!is.null(upper_bound) & any(x > upper_bound))
    stop('"', param_name, '" must be less than or equal to ', upper_bound)
  if (whole_num &&  !all(is.na(x)) && any(x %% 1 != 0))
    stop('"', param_name, '" must be whole number(s)')
}
