...
.check_response_input = function(x){
  ...
  x <- x[!is.na(x)]
  if (length(x) == 0) stop(deparse(match.call()[[2]]), ' must have at least a non "NA" value')
  if (!is.logical(x) & !all(x %in% c(NA,'T','F')) & !all(x %in% c(NA,0,1))) stop(deparse(match.call()[[2]]), ' must be a numeric vector containing only 0/1 values or a logical vector containing only T/F values')
}
