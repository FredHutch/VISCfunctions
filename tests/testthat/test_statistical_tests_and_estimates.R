...
test_that("cont_vs_binary testing", {
  ...
  expect_equal(object = cont_vs_binary(x = x[-(11:12)], y = y[-(11:12)], method = 'wilcox', paired = TRUE, verbose = T),
  ...
  expect_equal(object = cont_vs_binary(x = x[-(11:12)], y = y[-(11:12)], method = 't', paired = TRUE, verbose = T),
  ...
  expect_error(cont_vs_binary(x = x[-1], y = y), '"x" and "y" must be the same length')

  #Testing paired errors
  expect_error(cont_vs_binary(x = x, y = y, method = 't', paired = TRUE), 'When "Paired" = TRUE "y" can not have missing values')
  expect_error(cont_vs_binary(x = x[-(11:13)], y = y[-(11:13)], method = 't', paired = TRUE), 'When "Paired" = TRUE "y" must have the same number of samples for each level')
  ...
  expect_equal(object = cont_vs_binary(x = rep(1,22), y = y), expected = 1)
  expect_message(object = cont_vs_binary(x = rep(1,22), y = y, verbose = T),
  ...
  expect_equal(object = cont_vs_binary(x = x, y = rep(1,22)), expected = NA)
  expect_message(object = cont_vs_binary(x = x, y = rep(1,22), verbose = T),
