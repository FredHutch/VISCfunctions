...
test_that("pretty_pvalues testing various options (no errors)", {
  ...
  expect_equal(object = pretty_pvalues(c(0.00000001, NA, 0.05, 1), digits = 3, trailing_zeros = T, bold = T, output_type = "html"), expected = c("<span style=\" font-weight: bold;    \" ><0.001</span>", "---", "0.050", "1.000"))
  expect_equal(object = pretty_pvalues(c(0.00000001, NA, 0.05, 1), digits = 3, trailing_zeros = T, bold = T, output_type = "pandoc"), expected = c("**<0.001**", "---", "0.050", "1.000"))
  expect_equal(object = pretty_pvalues(c(0.00000001, NA, 0.05, 1), digits = 3, trailing_zeros = T, bold = T, output_type = "no_markup"), expected = c("<0.001", "---", "0.050", "1.000"))
