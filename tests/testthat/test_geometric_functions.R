context("geometric_functions")


test_that("geomean is the mean log value, exponentiated",{
  expect_equal(geomean(1:2), exp(sum(log(c(1:2)))/2))
  expect_equal(geomean(c(-1:100, NA)), exp(mean(log(c(1, 1, 1:100, NA)), na.rm = TRUE)))
...
test_that("geoquantile is the quantile log value, exponentiated",{
  ...
  expect_equal(geoquantile(1:10000000), exp(quantile(log(c(1:10000000)), type = 2)))
  expect_equal(unname(geoquantile(1:100)), exp(fivenum(x=log(1:100))))
})


test_that("geo functions return NA if na.rm = FALSE and NA are present",{
...
test_that("geo functions error when scalar data is provided",{
...
test_that("geo functions error when non-numeric vector provided",{
