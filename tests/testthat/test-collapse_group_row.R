...
test_that("Output columns are reordered and duplicated rows (based on columns pass) are removed",
    {
        ...
        sample_df <- data.frame(z = c(4, 5, 1, 2, 3),
                                x = c(2, 2, 1, 1, 1),
                                y = c(2, 3, 2, 3, 3))

        collapsed <- collapse_group_row(sample_df, x, y)
        expected_collasped <- data.frame(x = c(1, NA, NA, 2, NA),
                                         y = c(2, 3, NA, 2, 3),
                                         z = c(1, 2, 3, 4, 5))

        expect_equal(collapsed$z, 1:5)
        expect_equivalent(collapsed, expected_collasped)

        # testing without reordering
        collapsed_no_order <- collapse_group_row(sample_df, x, y, reorder_cols = FALSE)
        expect_equivalent(collapsed_no_order,
                          expected_collasped[, c(3,1,2)]
                          )
