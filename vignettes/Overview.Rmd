...
VISCfunctions is a collection of useful functions designed to assist in analysis
and creation of professional reports. The current VISCfunctions package can be
broken down to the following sections:
...
`trapz_sorted` is used to when you are not presorting your data so that it is
strictly increasing along the x-axis, or there are 'NA' values that are not
removed prior to estimating the area under a curve.
...
`pretty_pvalues` can be used on p values, rounding them to a specified digit
amount and using < for low p values, as opposed to scientific notation (i.e. "p
< 0.0001" if rounding to 4 digits), allows options for emphasizing p-values and
specific characters for missing. Currently latex, html, and pandoc (Word) are
supported.
...
`paste_tbl_grp` paste together information, often statistics, from two groups.
...
`pairwise_test_bin`, `pairwise_test_cont`, `pairwise_test_cor`, are
functions take will perform pairwise group (or any categorical variable)
comparisons. The function will goes through all pairwise comparisons and output
descriptive statistics and relevant p values, calling the appropriate testing
functions from the [Testing and Estimate
...
Simple example using `pairwise_test_cor`:

```{r, echo=TRUE, warning=FALSE, size='tiny'}

set.seed(1)
x_example <- c(1, 1, rnorm(48), rnorm(49, mean = 5), NA)
group_example <- c(rep(1, 25), rep(2, 25), rep(3, 25), rep(4, 25))
id_example <- c(rep(1:25, 4))

pairwise_test_cor(x_example, group_example, id_example, digits = 4)
...
Correlation comparisons using `pairwise_test_cor`:
...
antigen_testing <- exampleData_BAMA %>%
  ...
  group_modify(~ as.data.frame(
    pairwise_test_cor(x = .x$magnitude,
                      group = .x$antigen,
                      id = .x$pubID,
                      method = 'spearman',
                      n_distinct_value = 3))) %>% 
...
plot_data <-
 CAVD812_mAB %>%
  ...
  ggplot2::geom_step(direction = 'vh', lwd = .35) +
