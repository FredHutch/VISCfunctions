...
cor_test(x = cor_testing_data$`A1.con.env03 140 CF`, 
...
## Pairwise Comparison of Continuous Variables
...
antigen_testing <- exampleData_BAMA %>%
  filter(antigen %in% c('B.63521 D11gp120/293F', 'B.con.env03 140 CF', 'B.MN V3 gp70') & 
           group != 4) %>% 
  ...
  ungroup() %>% 
  select(-CorrTest) %>% 
  rename("Corr P Value" = CorrTest_Pretty)

kableExtra::kable(antigen_testing, 
