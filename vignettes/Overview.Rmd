...
`cor_test()` is used for comparing two continuous variables, with Pearson, Kendall, or Spearman methods. If the Spearman method is chosen and either variable has a tie, the approximate distribution is used in the `coin::spearman_test()` function. This is usually the preferred method over the asymptotic approximation, which is the method that `stats:cor.test()` uses in cases of ties.
