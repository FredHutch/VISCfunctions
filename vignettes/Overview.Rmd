...
The `exampleData_ICS`, `exampleData_ICS`, and `exampleData_ICS` datasets are assay example datasets used throughout this vignette and most examples in the `VISCfunctions` documentation. All example datasets have associated documentation that can be viewed using `?` (i.e `?exampleData_BAMA`).
...
exampleData_BAMA %>% 
  group_by(antigen, visitno) %>% filter(visitno != 0) %>% 
...
There are currently three completed functions, and four in development, designed to produce professional output that can easily printed in reports.
...
exampleData_BAMA %>% filter(visitno == 1) %>%  
...
Example of summary information to be pasted together:

```{r, echo=FALSE, warning=FALSE, size='tiny'}

summary_info <- exampleData_BAMA %>% filter(visitno == 1) %>%  
...
summary_info %>% select(antigen, Group1_max, Group1_mean, Group1_median, Group1_min, Group1_n)
...
group_testing <- exampleData_ICS %>%
  ...
  group_modify(~ as.data.frame(
    pairwise_test_bin(x = .$response, group = .$Group,
                      ...
                      latex_output = TRUE, verbose = FALSE))) %>%  ungroup() %>% 
...
timepoint_testing <- exampleData_ICS %>%
  ...
  group_modify(~ as.data.frame(
    pairwise_test_cont(x = .$PercentCellNet, group = .$Visit, id = .$pubID,
                       method = 'wilcox', paired = FALSE, num_needed_for_test = 3, 
